...
type OutputFormat = 'jpeg' | 'png' | 'webp' | 'csv' | 'svg' | 'json' | 'docx';
...
const FileConverter: React.FC<{ lang: Language }> = ({ lang }) => {
  ...
  const availableFormats: Record<string, OutputFormat[]> = {
    ...
    'application/pdf': ['png', 'jpeg', 'webp', 'docx'],
      ...
      if (inputType === 'image') {
        ...
        if (outputFormat === 'svg') {
          ...
          if (outputFormat === 'docx') {
            convertedBlob = await convertPdfToDocx(originalFile);
          } else {
  ...
  const convertPdfToDocx = async (file: File): Promise<Blob | null> => {
    return new Promise((resolve, reject) => {
      const reader = new FileReader();
      reader.onload = async (event) => {
        try {
          ...
          for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
            ...
            paragraphs.push(
              new Paragraph({
                children: [new TextRun({ text: `Page ${pageNum}`, bold: true, size: 24 })],
              ...
              headers: {
                default: new Header({
                  children: [new Paragraph({
                    ...
                    children: [new TextRun({ text: file.name, size: 16, color: '999999' })],
                  })],
                }),
              },
              footers: {
                default: new Footer({
                  children: [new Paragraph({
                    ...
                    children: [new TextRun({ text: 'Generated by SLIT.IO', size: 16, color: '999999' })],
                  })],
                }),
              },
              children: paragraphs,
  ...
  const getOutputFileName = () => {
    ...
    const ext = outputFormat === 'docx' ? 'docx' : outputFormat;
    return `${name}.${ext}`;
  }
        ...
        <div className={`mt-6 ${isLoading ? 'opacity-20' : ''}`}>
          <div className="flex flex-col md:flex-row items-center justify-center gap-4">
            ...
            <div className="text-center">
                ...
                {convertedUrl ? (
                    (outputFormat === 'csv' || outputFormat === 'json' || outputFormat === 'docx') ? (
